...
const R=rng(309), TOOLS=["New CRM","Sales sequencer","Lead scraper","KPI dashboard","Auto scheduling","Data enrichment"];
const MESSAGES=["Hi Marta…","Follow-up","Reminder","Re: demo","Did you see this?","Last attempt","Proposal v3","Another campaign","No reply","Bounced","Spam","Cold list"];
export const CARDS=[];
for(let i=0;i<36;i++){
...
const card=c=>`<div id="${c.id}" class="card s3-window ${c.lead?'tool':'message'}" data-layout-allow-overlap style="left:${c.x}px;top:${c.y}px;width:${c.w}px;height:${c.h}px">
  <div class="s3-windowbar"><i></i><i></i><i></i><span data-layout-allow-overlap>${c.lead?'TOOL':'MESSAGE'}</span></div>
...
const cal = `<div id="s3-cal" class="card cal p3d">
  <div class="calh"><span class="dot"></span><b>September</b><span class="calbtns"><span class="ghostb">Week</span><span class="btn or">Create</span></span></div>
  <div class="calg">
    <div class="corner"></div>${["MON 14", "TUE 15", "WED 16", "THU 17", "FRI 18"].map((d) => `<div class="dh">${d}</div>`).join("")}
    ${["09:00", "10:00", "11:00", "12:00", "13:00", "16:00"].map((h) => `<div class="th">${h}</div>` + "<div class=\"cell\"></div>".repeat(5)).join("")}
  </div>
</div>`;

export const html = `
<div id="s3-p1" class="phase bg-orange">${kin("s3-t1","So what do we do?",{pos:"center",color:"white"})}</div>
<div id="s3-p2" class="phase bg-light grid-light">
  ...
  ${kin("s3-t2","More tools",{pos:"center",size:96})}
  ${kin("s3-t5","More noise",{pos:"center",size:116})}
  <div class="stage">${cal}</div>
  ${label("s3-t6","Zero meetings")}
  <div id="s3-dark" class="veil dark"></div><div id="s3-dark2" class="layer" style="background:#f8f7ff;opacity:0;z-index:41"></div>
</div>
<div id="s3-p3" class="phase bg-light grid-light">${kin("s3-why", "Because in B2B", {pos:"center", size:92})}</div>
`;
export const css = `
