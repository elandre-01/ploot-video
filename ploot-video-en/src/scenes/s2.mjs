...
// ---------- 2.2–2.4 · chat abierto con signals alrededor · rebobinado (storyboard v2) ----------
const SIGS2 = [
  { p: PEOPLE.daniel, s: "Visits your website",  t: "12s", x: 126,  y: 190, from: -900 },
  { p: PEOPLE.lucia,  s: "Visits your profile", t: "34s", x: 1330, y: 162, from: 900 },
  { p: PEOPLE.pablo,  s: "Follows you",         t: "1m",  x: 110,  y: 578, from: -900 },
  { p: PEOPLE.elena,  s: "Shares your post", t: "2m",  x: 1340, y: 590, from: 900 },
  { p: PEOPLE.ivan,   s: "Saves your case study",   t: "3m",  x: 168,  y: 946, from: -900 },
  { p: PEOPLE.nuria,  s: "Requests pricing",     t: "4m",  x: 1330, y: 946, from: 900 },
];
...
const WARM = "Hi Pablo, thanks for following me! I saw you’re hiring for a sales role. It looks like your team is growing. Congratulations!";
...
const LONG = "Hi Pablo, I’m reaching out about our latest offer. I’m sure you’ll be interested: we’re offering a 10% discount. Would you be available for a call this week? Thanks and best regards.";
const SHORT = "Hi Pablo, I think we could help with your pipeline. Would a call this week work for you?";
...
const EV_NAMES = ["Madrid Trade Show", "Conference · Barcelona", "Exhibition · Valencia", "Summit Lisboa", "Expo Bilbao", "Forum · Sevilla", "Meetup Málaga", "Summit · Oporto",
  "Forum · Oporto", "Summit · Vigo", "Trade Show · Zaragoza", "Conference · Sevilla", "Exhibition · Alicante", "Summit Madrid", "Expo Valencia", "Forum · Bilbao",
  "Meetup Girona", "Summit · Málaga", "Trade Show · Murcia", "Conference · Vigo", "Exhibition · Palma", "Summit Barcelona", "Expo Coruña", "Forum · Granada",
  "Meetup Santander", "Summit · Valladolid", "Trade Show · Pamplona", "Conference · Salamanca", "Exhibition · Oviedo", "Summit Sevilla", "Expo Córdoba", "Forum · Cádiz",
  "Meetup Toledo", "Summit · Burgos", "Trade Show · León", "Conference · Logroño"];
const EV_TAGS = ["EVENT", "NEW EVENT", "PAST EVENT"];
const RE = rng(4102);
const IW = 440, IH = 600;
...
for (let i = 0; i < NI; i++) {
  ...
  EVENTS.push({ tag: EV_TAGS[i % 3], name: EV_NAMES[i % EV_NAMES.length], cost: "−" + (1900 + Math.round(RE() * 79) * 100).toLocaleString("en-GB") + " €",
...
const invoice = (e, i) => `<div class="card inv" id="s2-inv-${i}" data-layout-allow-overlap style="left:${e.x}px;top:${e.y}px;--rot:${e.rot}deg">
  ...
  <span class="lab" data-layout-allow-overlap>Total cost</span><span class="amt" data-layout-allow-overlap>${e.cost}</span><span class="pill"></span>
</div>`;

// ---------- tragaperras ----------
const REEL = ["nothing", "lead", "nothing", "referral", "nothing", "spam", "nothing", "customer"];
const reel = (id) => `<div class="reel" id="${id}"><div class="strip">${REEL.concat(REEL, REEL, REEL).map((w) => `<i class="${w === "referral" ? "ref" : ""}">${w}</i>`).join("")}</div></div>`;

export const html = `
<div id="s2-p1" class="phase bg-light">
  ${label("s2-t1", "Because you’re still selling", { extra: "center", size: 92 })}
</div>
...
<div id="s2-p2" class="phase bg-light grid-light">
  ...
  ${kin("s2-t2", "Like it’s 5 years ago")}
</div>
...
<div id="s2-p3" class="phase bg-light grid-light">
  ...
  <div id="s2-t3" class="kin top ink"><span class="line">${letters("Cold messages")}</span></div>
  ${kin("s2-t4", "Nobody opens", { pos: "top", size: 72 }).replace('<div id="s2-t4"', '<div id="s2-t4" style="top:900px"')}
</div>
...
<div id="s2-p5" class="phase bg-light grid-light">
  ...
  ${label("s2-t5", "Expensive events. No return.", { extra: "center", size: 72 }).replace('<div id="s2-t5"', '<div id="s2-t5" data-layout-allow-overlap')}
...
<div id="s2-p6" class="phase bg-light grid-light">
  ...
  <div class="stage">
    <div id="s2-slot" class="card slot p3d">
      <div class="sh"><span class="eyebrow">REFERRALS</span><span class="dots"><i></i><i></i></span></div>
      <div class="reels">${reel("s2-r1")}${reel("s2-r2")}${reel("s2-r3")}</div>
      <div class="sf"><span class="lab">No system</span><span class="pull" id="s2-pull">Spin</span></div>
    </div>
  </div>
  ${cursor("s2-cur3", { x: 1240, y: 1120, size: 96 })}
  ${kin("s2-t6", "And praying for referrals to show up")}
</div>
`;
