...
export const PEOPLE = {
  ...
  nuria:   { name: "Nuria Vidal",   role: "Finance Director",                  av: 5 },
  ...
  xavier:  { name: "Xavier Oliver", role: "Operations Director · Recuit",  av: 16 },
  marc:    { name: "Marc Jardí",    role: "Growth",                           av: 21 },
  angela:  { name: "Àngela Riba",   role: "Sales",                           av: 1 },
...
// ---------- kinetic text ----------
// words("People are deciding to buy") -> spans with .w
...
export function webHook(id, { x = 0, y = 0, cls = "" } = {}) {
  ...
  return `<div id="${id}" class="web hook ${cls}" style="left:${x}px;top:${y}px">
    ...
    <div class="wnav"><span class="brand"><b></b>Nordika</span><span class="links"><span>Product</span><span>Pricing</span><span>Case studies</span><span>Blog</span></span><span class="btn">Book a demo</span></div>
    <div class="whero">
      <div class="eyebrow">SMART LOGISTICS</div>
      <h2 data-layout-allow-overlap>B2B shipping. No surprises.</h2>
      ${bars([72, 58], "sub")}
      <div class="cta-row"><span class="btn">Book a demo</span><span class="ghost" data-layout-allow-overlap>View case studies</span></div>
      <div class="stats"><div><b>98.4%</b><span>on-time deliveries</span></div><div><b>−22 %</b><span>shipping cost</span></div><div><b>48 h</b><span>average onboarding</span></div></div>
    </div>
    <div class="wpricing">
      <div class="ph"><span class="eyebrow">PLANS</span><h3>Plans and pricing</h3></div>
      <div class="plans">
...
export function webPloot(id, { x = 0, y = 0, cls = "" } = {}) {
  return `<div id="${id}" class="web ploot ${cls}" style="left:${x}px;top:${y}px">
    ...
    <div class="wnav"><span class="brand"><img src="assets/brand/logo-dark.png" alt=""></span><span class="links"><span>Product</span><span>Pricing</span><span>Case studies</span></span><span class="right"><span class="btn">Book a demo</span><span class="in-ico" id="${id}-in">in</span></span></div>
    <div class="wgrid">
      <div class="whero compact">
        <div class="eyebrow">BUYING SIGNALS</div>
        <h2>Sell to people already looking at you</h2>
        ${bars([84, 62], "sub")}
        <div class="cta-row"><span class="btn">Get started</span><span class="ghost">See how it works</span></div>
        <div class="stats"><div><b>+38 %</b><span>meetings per month</span></div><div><b>−60 %</b><span>cold messages</span></div><div><b>12 min</b><span>response time</span></div></div>
      </div>
      <div class="wside">
        <div class="sh"><span class="eyebrow">SIGNALS · TODAY</span><span class="live">● live</span></div>
...
// ---------- chat / messages ----------
export function chatWindow(id, p, msg, { x = 0, y = 0, cls = "", sent = false, tick = false, subtitle = "CEO of Ploot", warm = false } = {}) {
  // warm = the Ploot-style message (storyboard 2.2 / 7.18): divider under the header, placeholder + orange «Send»
  const foot = warm
    ? `<div class="cf"><span class="ph">Write a message…</span><span class="sendbtn or">Send</span></div>`
    : `<div class="cf"><span class="tools"><i></i><i></i><i></i></span><span class="sendbtn">Send</span></div>`;
  return `<div id="${id}" class="card chat ${warm ? "warm" : ""} ${cls}" style="left:${x}px;top:${y}px">
    ...
    <div class="cb"><div class="bubble">${esc(msg)}</div>${tick ? `<div class="tick"><span class="receipt-sent">✓ Sent</span><span class="receipt-read">✓✓ Read</span></div>` : ""}</div>
    ${foot}
  </div>`;
}
