...
// ---------- the hook web, back as in frame 8.2: a landscape window, hero + «ENVÍOS · HOY» + stats ----------
const web8 = `<div id="s8-web" class="card nweb p3d">
  <div class="wbar"><i></i><i></i><i></i></div>
  <div class="wnav"><span class="brand" data-layout-allow-overlap><b></b>Nordika</span><span class="links" data-layout-allow-overlap><span>Producto</span><span>Precios</span><span>Casos</span><span>Blog</span></span><span class="btn" data-layout-allow-overlap>Pedir demo</span></div>
  <div class="nbody">
    <div class="nhero"><div class="eyebrow" data-layout-allow-overlap>LOGÍSTICA INTELIGENTE</div><h2 data-layout-allow-overlap>Envíos B2B sin sorpresas</h2><div class="cta-row"><span class="btn" data-layout-allow-overlap>Pedir tarifa</span><span class="ghost" data-layout-allow-overlap>Ver cómo funciona</span></div></div>
    <div class="ncard"><span class="eyebrow ink" data-layout-allow-overlap>ENVÍOS · HOY</span>${[["MAD-0912", "en reparto"], ["BCN-0447", "en almacén"], ["VLC-0231", "entregado"]].map(([c, st]) => `<div class="nrow"><i></i><b data-layout-allow-overlap>${c}</b><span data-layout-allow-overlap>${st}</span></div>`).join("")}</div>
  </div>
  <div class="nstats">${[["98,4 %", "entregas a tiempo"], ["−22 %", "de coste por envío"], ["48 h", "de tránsito medio"]].map(([v, l]) => `<div><b data-layout-allow-overlap>${v}</b><span data-layout-allow-overlap>${l}</span></div>`).join("")}</div>
</div>`;
...
const vprofile = `<div id="s8-prof" class="card vprof">
  ...
  <div class="alert" id="s8-alert"><i></i><div><b>Contactar ahora</b><span>Está en tu web por tercera vez en 24 h</span></div><span class="btn or small" id="s8-contact">Contactar</span></div>
  <div class="sigs">
    ${[["Página de precios", "hace 2 min"], ["Caso de cliente · industria", "hace 9 min"], ["Comparó con tu competencia", "ayer"]].map(([sg, t]) => `<div class="srow2"><i></i><span>${sg}</span><em>${t}</em></div>`).join("")}
  </div>
</div>`;

export const html = `
<div id="s8-p1" class="phase bg-light grid-light">
  ${kin("s8-t1", "Deja de perseguir clientes fríos", { pos: "center", size: 92 })}
  ${kin("s8-t2", "Aparece cuando ya quieren comprarte")}
  <div class="stage">${web8}</div>
  ${vtag("s8-v2", PEOPLE.javier, "Javier Durán", "Brand Strategist", "#f43600", 1080, 440)}
  ${vtag("s8-v1", PEOPLE.irene, "Irene Salas", "Dir. de Operaciones", "#2d5bff", 690, 720)}
  ${vtag("s8-v3", PEOPLE.lucia, "Lucía Ferrer", "Compras", "#1fa463", 1330, 790)}
  ...
  <div id="s8-ctawrap" class="ctawrap">
    <div class="ctabox">
      ...
      <span class="cta" id="s8-cta">Agenda una demo</span>
    </div>
    <span class="sub" id="s8-sub">y analizamos tu caso</span>
  </div>
