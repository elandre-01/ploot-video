...
// ---------- Ploot dashboard ----------
const NAV1 = ["Leads", "Campañas", "Bandeja"], NAV2 = ["Posts", "Calendario", "Analíticas", "Conocimiento"];
const TASKS = [["Recap de la semana", "Ver →"], ["Cuéntanos tu semana", "Responder →"], ["Publicar posts semanales", "Ver semana →"]];
const METRICS = [["LEADS GENERADOS", "157", "+14 en 24 h", true], ["PIPELINE ACTIVO", "24", "+7 esta semana", true], ["IMPRESIONES", "460.965", "+303 % vs mes ant.", true], ["ENGAGEMENT", "1,2 %", "+53 % vs mes ant.", true]];
const RETOS = [["Publicar 2 posts", "+2"], ["Sumar 4.000 impresiones", "+2"], ["Ganar 20 seguidores", "+2"]];
const dash = `<div id="s7-dash" class="card dash p3d">
  <div class="dside">
    <div class="dlogo"><i></i>Ploot</div>
    <div class="dnav on"><i></i>Inicio</div>
    <div class="dsec">PIPELINE</div>
    ${NAV1.map((n) => `<div class="dnav"><i></i>${n}</div>`).join("")}
    <div class="dsec">CONTENIDO</div>
    ${NAV2.map((n) => `<div class="dnav"><i></i>${n}</div>`).join("")}
  </div>
  <div class="dmain">
    <div class="dtop"><span class="dtime" data-layout-allow-overlap>Martes, 6 de septiembre · Álvaro</span></div>
    <div class="dgrid">
      <div class="dlead">
        <h3 data-layout-allow-overlap>157 leads esperan tu decisión</h3>
        <div class="drow">
          ${avatar(PEOPLE.xavier.av, { size: 52, live: false })}
          <div class="dwho">
            <b>${PEOPLE.xavier.name}<span class="inb">in</span></b>
            <span>Co-fundador · Responsable de Recruitment</span>
            <span class="dtags"><em class="or">Comentó perfil de referencia</em><em>Recursos Humanos</em></span>
            <span class="dscore"><em>82</em>Ver perfil →</span>
          </div>
          <span class="dacts"><i class="gr"><svg viewBox="0 0 24 24" aria-label="Aceptar lead"><path d="m5 12 4 4L19 6"/></svg></i><i class="or"><svg viewBox="0 0 24 24" aria-label="Descartar lead"><path d="m7 7 10 10M17 7 7 17"/></svg></i></span>
        </div>
      </div>
      <div class="dtasks"><span class="eyebrow ink">TAREAS DE HOY</span>${TASKS.map(([t, a]) => `<div class="task"><i></i><span data-layout-allow-overlap>${t}</span><em data-layout-allow-overlap>${a}</em></div>`).join("")}</div>
    </div>
    ...
    <div class="dgrid2">
      <div class="drank"><span class="rh"><span class="eyebrow ink">RANKING</span><em>Ver podium →</em></span>${[[PEOPLE.marc, "24 puntos", "+24", false], [PEOPLE.angela, "18 puntos", "+18", true]].map(([p, pt, v, hi]) => `<div class="rrow ${hi ? "hi" : ""}">${avatar(p.av, { size: 34, live: false })}<div><b data-layout-allow-overlap>${p.name}</b><span data-layout-allow-overlap>${pt}</span></div><em data-layout-allow-overlap>${v}</em></div>`).join("")}</div>
      <div class="dretos"><span class="rh"><span class="eyebrow ink">RETOS SEMANALES</span><em class="c">0/3</em></span>${RETOS.map(([t, v]) => `<div class="reto"><i></i><span data-layout-allow-overlap>${t}</span><em data-layout-allow-overlap>${v}</em></div>`).join("")}</div>
    </div>
  </div>
...
const cprofile = `<div id="s7-cprof" class="card cprof p3d">
  ...
  <div class="cbody"><b>${PEOPLE.clara.name}</b><span class="role">${PEOPLE.clara.role}</span><div class="fol"><b id="s7-fol">1.240</b> seguidores</div><div class="ptabs"><span class="on">Publicaciones</span><span>Actividad</span><span>Sobre mí</span></div></div>
</div>`;
...
const LEADS = [
  ...
  [PEOPLE.xavier, 3, "Director de Operaciones", "Recuit"],
];
const leads = `<div id="s7-leads" class="leads p3d">
  ${LEADS.map(([p, f, title, co], i) => `<div class="lrow ${f === 3 ? "hot" : ""}" id="s7-lr-${i}" data-f="${f}">
    ...
    <span class="lst"><span class="old" data-layout-allow-overlap>Lead potencial</span><span class="new" data-layout-allow-overlap>En contacto</span></span>
    <span class="ideal">Momento ideal</span>
  </div>`).join("")}
</div>`;

// One recipient, one sent message. Paragraphs reveal as meaningful units.
const MESSAGE = [
  "Hola Noemí, gracias por |empezar a seguirme|.",
  "He visto que tenéis una |vacante comercial abierta|. Con equipos como el vuestro hemos conseguido |12 reuniones de venta| en un mes.",
...
const conversation = `<div id="s7-a" class="card chan p3d in">
  ...
  <div class="thread">
    <span class="chat-date">Hoy · 10:24</span>
    <div class="msg me" data-side="outgoing" id="s7-a-msg"><div class="cht">${messageHtml}</div><span class="meta sent" id="s7-a-stamp">✓✓ Enviado · 10:24</span></div>
    <div class="msg them" data-side="incoming" id="s7-a-reply">${avatar(PEOPLE.noemi.av,{size:38,live:false})}<div><div class="bub2">Sí, cuéntame. ¿Te encaja el jueves a las 11:00?</div><span class="meta">Noemí · 10:33</span></div></div>
  </div>
  <div class="chat-compose"><span>Escribe un mensaje…</span><span class="chat-attach">+</span><span class="chat-send" id="s7-send">${sendIcon}</span></div>
</div>`;
...
const week = `<div id="s7-week" class="card week p3d">
  <div class="wh"><span class="eyebrow ink">TU SEMANA</span><b>Septiembre</b></div>
  <div class="wg">${["LUN 14", "MAR 15", "MIÉ 16", "JUE 17", "VIE 18"].map((d) => `<div class="dh">${d}</div>`).join("")}${Array.from({ length: 30 }, (_,i) => `<div class="cell" style="grid-column:${i%5+1};grid-row:${Math.floor(i/5)+2}"></div>`).join("")}
    ${MEET.map(([d, r, t, n], i) => `<div class="meet" id="s7-meet-${i}" style="grid-column:${d+1};grid-row:${r+2}"><b>${t} · Demo</b><span>${n}</span></div>`).join("")}
  </div>
  <div class="wf2"><span class="chip green" id="s7-agend">● Demos confirmadas</span><span class="resp"><b id="s7-resp">0</b> reuniones esta semana</span></div>
</div>`;

export const html = `
<div id="s7-p1" class="phase bg-light grid-light">
  <div id="s7-title-scrim" class="layer"></div>
  ${kin("s7-t0", "Uno", { pos: "center", size: 132 })}
  ${kin("s7-t1", "Convertimos el conocimiento de tu equipo")}
...
<div id="s7-p2" class="phase bg-light grid-light">
  ${kin("s7-t2", "Convertimos el conocimiento de tu equipo")}
  ${label("s7-t3", "En audiencia")}
...
<div id="s7-p3" class="phase bg-light grid-light">
  ${kin("s7-t4", "Dos", { pos: "center", size: 132 })}
  ${kin("s7-t5", "Detectamos cada señal de compra")}
  ${label("s7-t6", "Y actuamos en minutos")}
...
<div id="s7-p4" class="phase bg-light grid-light">
  ${kin("s7-t7", "Tres", { pos: "center", size: 132 })}
  ${label("s7-t8", "Contactamos al lead")}
  ${label("s7-t9", "En el momento justo")}
  <div id="s7-chips" class="chips"><span class="chip green" id="s7-chip1">● Mensaje enviado</span></div>
  ${label("s7-t10", "Para agendarte reunión")}
