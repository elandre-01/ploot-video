...
const INBOX = [[PEOPLE.sofia, "10:24"], [PEOPLE.daniel, "09:58"], [PEOPLE.lucia, "09:31"], [PEOPLE.pablo, "09:02"], [PEOPLE.nuria, "Ayer"], [PEOPLE.ivan, "Ayer"]];
const inbox = `<div id="s4-inbox" class="card inbox p3d">
  <div class="ih"><span class="eyebrow ink">BANDEJA DE ENTRADA</span><span class="cnt"><i></i>6 LEÍDOS</span></div>
  ${INBOX.map(([p, t], i) => `<div class="irow" id="s4-ir-${i}">${avatar(p.av, { size: 84, live: false })}<div class="who"><b>${esc(p.name)}</b><span>${esc(p.role)}</span></div><span class="tm">${t}</span><span class="rd" id="s4-rd-${i}"><span class="ticks">✓✓</span> LEÍDO</span></div>`).join("")}
</div>`;

// relationship line: five phase cards
export const PH = [
  { t: "HACE 3 MESES", s: "Visitó tu web", c: "Ya te conoce" },
  { t: "HACE 2 MESES", s: "Te siguió", c: "Ya te sigue" },
  { t: "HACE 3 SEMANAS", s: "Tiene una ronda de inversión", c: "Oportunidad detectada" },
  { t: "HACE 6 DÍAS", s: "Guardó tu caso de cliente", c: "Ya confía en ti" },
  { t: "HOY", s: "COMPRA", c: "" },
];
...
<div id="s4-p1" class="phase bg-white">
  ${kin("s4-t1", "Nadie compra a un desconocido", { pos: "center", size: 92 })}
  <div class="stage">${inbox}</div>
</div>

<div id="s4-p2" class="phase bg-orange">
  ${kin("s4-t2", "La gente compra a quien ya conoce", { pos: "center", color: "white", size: 92 })}
</div>

<div id="s4-p3" class="phase bg-white">
  ${kin("s4-t3", "A quien ya siguen")}
  ${kin("s4-t4", "En quien ya confían")}
